...
echo "# Project Structure" > .cursor/rules/structure.md
echo "" >> .cursor/rules/structure.md
echo "\`\`\`" >> .cursor/rules/structure.md

# Check if tree command is available
if command -v tree &> /dev/null; then
  # Use tree command for better visualization
  git ls-files --others --exclude-standard --cached | tree --fromfile -a >> .cursor/rules/structure.md
  ...
  while read -r item; do
    ...
    # Output the item
    echo "${prefix}${connector}${name}" >> .cursor/rules/structure.md
...
echo "Project structure has been updated in .cursor/rules/structure.md"